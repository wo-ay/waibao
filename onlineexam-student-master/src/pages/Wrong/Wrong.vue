<template>
  <div class="wrong">
    <HeaderTop title="错题查看" style="background-color: #4ab8a1"></HeaderTop>
      ...
      <mt-loadmore :top-method="loadTop" ref="loadmore">
        <div class="wrong_list" v-for="(item, index) in wrongPapersList" :key="item.paperId">
          <div class="wrong_list_item" :class="{'corner_new':index == 0}">
            ...
            <div class="wrong_btn">
              <div>
                <mt-button size="small" type="danger" style="border-radius: 15px" @click.native="toWrongDetail(item.paperId)" :disabled="item.score == null">查看错题记录</mt-button>
              </div>
              <div>
                <mt-button size="small" type="primary" style="background-color: #4ab8a1;border-radius: 15px" @click.native="toScoreDetail(item.paperId)" :disabled="item.score == null">查看成绩报告</mt-button>
              </div>
            </div>
          </div>
        </div>
